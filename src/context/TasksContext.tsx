...
import { api } from '../api/client'
  ...
  function tasksReducer(state: Task[] = [], action: Action): Task[] {
    switch (action.type) {
      ...
      case 'UPDATE_TASK': return state.map(t => t.id === action.payload.id ? action.payload : t)
  ...
  const TasksDispatchCtx  = createContext<{ addTask: (title: string) => any } | undefined>(undefined)
  const FilterStateCtx    = createContext<{ showCompleted: boolean } | undefined>(undefined)
  const FilterDispatchCtx = createContext<{ toggle: () => void }            | undefined>(undefined)
  ...
  export function TasksProvider({ children }: { children: ReactNode }) {
    ...
    const addTaskThunk = (title: string): ThunkAction<State, Action> =>
  ...
  export function useTasksLoading() {
    ...
    if (v === undefined) throw new Error('useTasksLoading must be inside TasksProvider')
    return v
  }
  export function useTasksActions() {
    const v = useContext(TasksDispatchCtx)
    if (v === undefined) throw new Error('useTasksActions must be inside TasksProvider')
    return v
  }
  export function useFilterState() {
    const v = useContext(FilterStateCtx)
    if (v === undefined) throw new Error('useFilterState must be inside TasksProvider')
    return v
  }
  export function useFilterActions() {
    const v = useContext(FilterDispatchCtx)
    if (v === undefined) throw new Error('useFilterActions must be inside TasksProvider')
    return v
  }